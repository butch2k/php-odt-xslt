...
class Odt2Html {
	...
	public function __construct($odtfile='test/test.odt', $xsltfile='odt2html.xsl')
	{
		$xslFile = "odt";
		$xslDoc = new DOMDocument();
		$xslDoc->load($xsltfile);
		//$xslDoc->loadHTML(self::XSL);

		$odtZipfilename = "zip://" . dirname(__FILE__) . "/" . $odtfile;
	...
	static public function factory($odtfile='test/test.odt', $xsltfile='odt2html.xsl')
	{
		return new self($odtfile, $xsltfile);
	}
	...
	const XSL =
		...
		<div class="drawframe" width="{@svg:width}" heigth="{@svg:height}">
			<xsl:apply-templates/>
		</div>
		</xsl:template>
